...
public class SM4 {
	 private static final int ENCRYPT = 1;
	    ...
	    static byte[] Sbox = { (byte) 0xd6, (byte) 0x90, (byte) 0xe9, (byte) 0xfe,
	    ...
	    private static final byte[] lowerHex = {
	            0x30, 0x31, 0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,
	            0x61,0x62,0x63,0x64,0x65,0x66
	    };


	    public static byte[] encrypt(byte[] in, int inLen, byte[] key) {
	        int roundLen = inLen % BLOCK == 0 ? inLen : (inLen / 16 + 1) * 16;
	        byte[] input = new byte[roundLen];
	        byte[] output = new byte[roundLen];
	        System.arraycopy(in, 0, input, 0, inLen);
	        Arrays.fill(input, inLen, roundLen, (byte)32);
	        sms4(input, roundLen, key, output, ENCRYPT);
	        return encodeHex(output);
	    }

	    private static byte[] encodeHex(byte in[]) {
	        int len = in.length;
	        byte hex[] = new byte[len << 1];

	        int i = 0;
	        for(int j = 0; i < len; ++i) {
	            hex[j++] = lowerHex[(240 & in[i]) >>> 4];
	    ...
	    private static byte[] decodeHex(byte in[], int length) {
	        int len = length;
	    ...
	    public static byte[] decrypt(byte[] in, int inLen, byte[] key) {
	    ...
	    public static String doEncrypt(String enc,String mKey){
	        byte input[] = enc.getBytes(Charset.forName("utf-8"));
	        byte key[] = mKey.getBytes(Charset.forName("UTF-8"));
	        byte output[] = encrypt(input, input.length, key);
	        return new String(output,Charset.forName("utf-8"));
	    }

	    public static String doDecrypt(String dec,String mKey){
	        byte output[] = dec.getBytes(Charset.forName("UTF8"));
	        byte key[] = mKey.getBytes(Charset.forName("UTF-8"));
	        return new String(decrypt(output, output.length, key), Charset.forName("utf-8"));
	    }
}
